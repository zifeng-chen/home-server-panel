...
### 改进
- **云服务凭据脱敏显示** — 输入框直接显示脱敏值(如 `AKIDxxxx****`/`****`)替代"已配置"标签；新值保存后自动更新显示
- **推送测试按钮** — PushPlus Token 旁新增"测试推送"按钮，调用 `POST /api/notify/test` 实时验证推送通道
- **DNS 服务商下拉选择** — 从纯文本输入改为分组下拉框(国内4家+国际6家)，中文标签+技术ID双显示
- **版本号 0.8.2** — 全局版本号统一更新

### 修复
- Nginx 管理页：状态信息与操作按钮合并到同一卡片不再分散
- Nginx 管理页：反向代理标题+规则数量+操作按钮水平并行一行
- 暗色模式：el-table 加载遮罩白底→暗色 `rgba(10,14,24,0.75)`
- 暗色模式：el-select-dropdown/el-popper 下拉菜单暗色背景
- 设置页：云服务凭据标签 label-width 140→170px + `white-space: nowrap` 防换行
...
- **Element Plus 跟随** — `main.ts` 根据语言自动加载 Element Plus 中/英 locale
- **全页面标题** — 11 个页面标题 + 侧栏菜单项全部 i18n 化
- **Login / Dashboard / Settings** — 完整双语支持（表单、提示、按钮文字）
- **暗色按钮** — TopBar 暗色/亮色切换按钮已在代码中（☀️/🌙 图标），部署后硬刷新即可见
...
### 修复
- **反向代理 SSL 证书选择器**: 勾选启用 SSL 后自动弹出证书选择下拉框，支持域名匹配✅标记置顶和手动选择
- **PushPlus 通知修复**: 反向代理增删改启停通知现在正确显示域名和目标主机
- **手动 DNS 证书签发**: 系统设置 ACME→手动 DNS 模式支持两阶段签发（获取 TXT → 确认后完成）
- **CA 提供商选择器**: 证书申请弹窗支持 ZeroSSL / Let's Encrypt 切换

### 改进
- 版本号升级至 0.7.3-beta
- 反向代理 SSL 选择器自动联动来源域名变化（blur 事件刷新证书列表）
- 编辑反向代理时自动预选已配置的证书

---

# v0.7.2-beta (2026-06-17)

### ✨ 新增功能
- **SSL 证书到期通知可配置天数** - 系统设置中新增 `CERT_EXPIRE_WARN_DAYS` 字段，可在 PushPlus 区域设置证书到期前通知天数（默认 30 天）
- **PM2 页面整合进程总览** - 移除独立「进程总览」页面，合并至 PM2 页面，统一展示 PM2 + Docker + 系统服务，类型标签区分

### 🐛 问题修复
- **SSH 终端断开按钮** - 修复 SSH 连接成功后断开按钮不出现的问题，`_sshConnect` 重连后完整渲染页面
- **资源监控图表扩大 2 倍** - CPU/内存/网络/负载图 Canvas 高度 160→320，字号和间距同步放大，显示更清晰
