...
window.installAcme = async () => {
  const email = prompt('请输入 Let\'s Encrypt 联系邮箱（用于证书到期通知）：', localStorage.getItem('acmeEmail') || '');
  if (!email) return;

  localStorage.setItem('acmeEmail', email);
  Utils.notify('正在安装 acme.sh，请稍候...', 'info');

  const res = await Api.post('/cert/acme/install', { email });
  if (res.success) {
    Utils.notify('✅ ' + res.message, 'success');
    loadCert();
  } else {
...
window.loadCert = async function() {
  ...
  try {
    ...
    if (installBtn) {
      installBtn.style.display = (acmeRes.data && acmeRes.data.installed) ? 'none' : 'inline-flex';
    }
  } catch {}
};