...
HSP 有两种安装方式：**首次部署用引导安装**（浏览器点点就完成），**已有环境直接启动**。

## 方式一：引导安装（推荐首次部署）

启动服务后，浏览器打开面板地址，会自动跳转到安装向导页面：

```
http://localhost:3456 → 自动跳转 → /install.html
```

三步完成：

1. **数据库配置** — 选 SQLite（零配置）或填 MySQL 连接信息并测试连接
2. **管理员账号** — 设置用户名和密码（替换默认的 admin/admin123）
3. **完成** — 系统初始化后自动跳转登录页

> 引导安装页的代码是 `public/install.html`，对应的后端 API 在 `/api/setup/*`（无需认证）。

## 方式二：直接启动（已有 `.env` 或数据库文件）
